...
def charger_et_aggreguer_donnees(root_dir):
    ...
    Parcourt la structure de dossiers sous root_dir.
    Chaque dossier représente une salle et contient plusieurs fichiers CSV répartis dans des sous-dossiers (positions).
    Pour chaque fichier CSV, on lit les données, on détecte et remplace les valeurs manquantes dans "RSSI" 
    (pour chaque "Mac Address", les valeurs manquantes sont remplacées par la moyenne des autres mesures pour ce MAC).
    On agrège ensuite les mesures en regroupant par "Mac Address" et en collectant toutes les valeurs "RSSI" dans une liste.
    ...
    for salle in os.listdir(root_dir):
        ...
        if os.path.isdir(chemin_salle):
            ...
            # Parcourir récursivement tous les fichiers CSV à l'intérieur des sous-dossiers
            for root_path, _, files in os.walk(chemin_salle):
                for fichier in files:
                    if fichier.lower().endswith('.csv'):
                        chemin_fichier = os.path.join(root_path, fichier)
                        try:
                            df = pd.read_csv(chemin_fichier, encoding="utf-8")
                            # Vérifier la présence des colonnes requises
                            if "Mac Address" in df.columns and "RSSI" in df.columns:
                                # Convertir la colonne "RSSI" en numérique (les erreurs produiront des NaN)
                                df["RSSI"] = pd.to_numeric(df["RSSI"], errors="coerce")
                                # Pour chaque Mac Address, remplacer les valeurs manquantes par la moyenne des autres mesures
                                df["RSSI"] = df.groupby("Mac Address")["RSSI"].transform(lambda x: x.fillna(x.mean()))
